...
describe("ScheduleDateParams", () => {
  it("keeps the padded segments the URL carried", async () => {
    await expect(
      parseParams(ScheduleDateParams, params({ year: "2026", month: "08", date: "27" })),
    ).resolves.toEqual({ year: "2026", month: "08", date: "27" });
  });

  it("accepts a leap day the year actually has", async () => {
    await expect(
      parseParams(ScheduleDateParams, params({ year: "2028", month: "02", date: "29" })),
  ...
  it.each([
    ...
    await expect(parseParams(ScheduleDateParams, params(day))).rejects.toThrow();
  });
});
